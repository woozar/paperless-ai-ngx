...
export function ThemeSettings() {
  ...
  return (
    <div className="grid gap-8 lg:grid-cols-[1fr_2fr]">
      ...
      <div className="space-y-1">
        <h3 className="text-base font-medium">{t('theme.title')}</h3>
      ...
      <Card className="bg-gray-100 p-6 dark:bg-gray-800/50">
        <CardContent className="space-y-6 p-0">
          <div className="space-y-4">
            <Label className="block">{t('theme.mode.title')}</Label>
              ...
              <SelectContent>
                {THEME_OPTIONS.map((option) => (
                  <SelectItem key={option} value={option}>
                    {t(`theme.mode.values.${option}`)}
                  </SelectItem>
                ))}
              </SelectContent>
            </Select>
            <p className="text-muted-foreground text-sm">{t('theme.mode.description')}</p>
          </div>
        </CardContent>
      </Card>
    </div>
